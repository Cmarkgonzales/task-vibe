...
const TaskCard = ({ task, onOpenCard }) => {
    ...
    const getBorderClass = (priority) => {
        return clsx(
            'p-4 rounded-lg bg-white shadow-sm relative task-card card-bg  border-l-4 cursor-pointer',
            {
                high: 'border-danger',
    ...
    return (
        <div
            className={getBorderClass(task.priority)}
            onClick={onOpenCard}
        >
            ...
            {task.description && (
                <p className="text-xs mb-3 text-text-light line-clamp-3">{task.description}</p>
            )}
